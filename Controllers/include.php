...
function apply_list_filters(& $request, $table, $field_unique_id)
	...
	foreach ($request["filters"] as $filt) // Only apply filters that are by asked for by the client-side request. 
			foreach ($filt as $source_fieldUniqueId) //for each client-picked filterING control...
				{
			/*	$source_field = $_SESSION['_AcField'][$requester_page][$source_fieldUniqueId];
				$source_field_filters = $source_field["filtered_fields"];
				var_dump($source_field_filters);*/
				
				$source_field = AcField::instance_from_id($source_fieldUniqueId) or json_error("Library Error #3"); //In event of a nonexistent source_fieldUniqueId passed, terminate.
				$source_field_filters = $source_field->filtered_fields;
