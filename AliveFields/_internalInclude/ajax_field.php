...
function acField_Controller(& $fake_this) {
    ...
    if (($dataRequest['action'] === "save")) {
        ...
        if ($this_field->savable != AcField::SaveYes)
    ...
    elseif (($dataRequest['action'] === "hardcoded_load") || ($dataRequest['action'] === "dynamic_load")) {
        ...
        if ($dataRequest['action'] === "dynamic_load") {
            $where_clause['key_piece'] = _AcField_escape_table_name($this_field->bound_table)
                . "." . _AcField_escape_field_name($this_field->bound_pkey) . " = " 
                . _AcField_escape_field_value($dataRequest['primaryInfo'][1]);
            ...
            if (count($_SESSION['_AcField'][$requestingPage][$sourceUniqueId]['filters'])) {
                ...
                $join_clause[] = $source_field['bound_table'] . " ON " . 
                        _AcField_escape_table_name($this_field->bound_table) 
                        . "." . _AcField_escape_field_name($this_field->bound_pkey) 
                        . " = " . _AcField_escape_table_name($source_field['bound_table']) 
                        . "." . _AcField_escape_field_value($source_field["bound_pkey"]);
