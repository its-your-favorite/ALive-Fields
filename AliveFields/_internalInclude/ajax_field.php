...
function acField_Controller(& $fake_this, $request) {
    ...
    if (($request['action'] === "save")) {
        ...
        if ($this_field_session['type'] == 'multi')
            verify_control_could_contain_value($fake_this, $requestingPage, 
                    $fieldUniqueId, ($x[1]), "optionValue") or json_error("expectedError");
    }
    ...
    elseif ($request['action'] === "save") {//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        $security_check = $fake_this->adapter->call_query_read($sql); // By seeing how many rows our current where clause selects we can add an additional level of security.
...
// technically this is redundant with the inclusion of "loaded_where_clause" in the actual UPDATE statement. 
        ...
        $fake_this->adapter->call_query_write($sql, 1);
